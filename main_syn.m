...
% M = normr(M);
% M = rand(100,50);
load('/Users/imote/Documents/MATLAB/IJCAI/M_r_1.mat');
% M = M ./ max(M,[],2);
[B,I] = maxk(M,topk,2);
% I = sort(I, 2);
...
% w_f = rand(100,25);
% d_f = rand(100,25);
load('/Users/imote/Documents/MATLAB/IJCAI/w_f_r_1.mat');
load('/Users/imote/Documents/MATLAB/IJCAI/d_f_r_1.mat');
...
%         alg_cr(1) = sum(sum(wd_f .* x_samp)) / opt + alg_cr(1);
        alg_cr_1(2) = sum(sum(w_f .* x_samp)) / opt_1 + alg_cr_1(2);
        alg_cr_2(2) = sum(sum(d_f .* x_samp)) / opt_2 + alg_cr_2(2);
...
%         alg_cr(1) = sum(sum(wd_f .* x_samp)) / opt + alg_cr(1);
        alg_cr_1(3) = sum(sum(w_f .* x_samp)) / opt_1 + alg_cr_1(3);
        alg_cr_2(3) = sum(sum(d_f .* x_samp)) / opt_2 + alg_cr_2(3);
        ...
        %%%Uniform
        B_c_i = c_i;
        x_uniform = zeros(user_num,bundle_num_per_user);
        %arrival at time t
        for t = 1:numel(j_seq)
            j_id = j_seq(t);
            bundle_list = b_f(j_id,:);
            w_f_j = w_f(j_id,:);
            safe_index_list = zeros(1,bundle_num_per_user);
            %%check out safe set
            for i=1:bundle_num_per_user
    %             [v, I] = max(w_f_j);
    %             index = randi
    ...
    %                 break;
            ...
            safe_index_list = find(safe_index_list > 0);
            if (numel(safe_index_list) > 0)
                safe_index = safe_index_list(randperm(numel(safe_index_list),1));
                movie_list = cell2mat(bundle_list(safe_index));
                B_c_i(movie_list) = B_c_i(movie_list) - 1;
                x_uniform(j_id,safe_index) = x_uniform(j_id,safe_index) + 1;
            end
        end
        alg_cr_1(4) = sum(sum(w_f .* x_uniform)) / opt_1 + alg_cr_1(4);
        alg_cr_2(4) = sum(sum(d_f .* x_uniform)) / opt_2 + alg_cr_2(4);
...
%         alg_cr(3) = sum(sum(wd_f .* x_greedy_e)) / opt + alg_cr(3);
    alg_cr_1(5) = sum(sum(w_f .* x_greedy_e)) / opt_1 + alg_cr_1(5);
    alg_cr_2(5) = sum(sum(d_f .* x_greedy_e)) / opt_2 + alg_cr_2(5);
...
%         alg_cr(4) = sum(sum(wd_f .* x_greedy_d)) / opt + alg_cr(4);
    alg_cr_1(6) = sum(sum(w_f .* x_greedy_d)) / opt_1 + alg_cr_1(6);
    alg_cr_2(6) = sum(sum(d_f .* x_greedy_d)) / opt_2 + alg_cr_2(6);
...
alg_cr_1(1:4) = alg_cr_1(1:4) / (EXP_TIMES * SAMPLE_TIMES);
alg_cr_1(5:6) = alg_cr_1(5:6) / EXP_TIMES;

alg_cr_2(1:4) = alg_cr_2(1:4) / (EXP_TIMES * SAMPLE_TIMES);
alg_cr_2(5:6) = alg_cr_2(5:6) / EXP_TIMES;
